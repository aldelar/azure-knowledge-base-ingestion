"""Render HTML to PDF with image markers via Playwright Chromium.

Replaces ``<img>`` tags with visible text markers (``[[IMG:<filename>]]``)
and renders the marked-up HTML to PDF.  The markers are rendered in
normal-size black text so Mistral OCR reliably preserves them in the
extracted Markdown.  The actual images are **not** included in the PDF —
we have the source files in the staging directory.
"""
