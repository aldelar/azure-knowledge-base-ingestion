"""Extract image markers from OCR markdown.

During PDF rendering, each ``<img>`` tag is replaced with a visible text
marker ``[[IMG:<filename>]]``.  Mistral OCR preserves these markers in the
extracted Markdown.  This module scans for them and returns an ordered list
of source filenames found in the document.
"""
