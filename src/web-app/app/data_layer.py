...
container (partition key ``/conversationId``).  Each document represents
one conversation thread with embedded steps and elements arrays.
...
::
    ...
    {
        "id": "<thread-id (UUID)>",
        "conversationId": "<thread-id>",   # partition key
        "userId": "<user-id>",
...
class CosmosDataLayer(BaseDataLayer):
    ...
    The layer stores conversation threads as documents in the
    ``agent-sessions`` container, partitioned by ``/conversationId``
    ...
    def _read_thread_doc(self, thread_id: str) -> dict | None:
        """Read a thread document via point read.

        The partition key is ``/conversationId`` which equals the
        ``thread_id``, so a direct point read is always possible.
        """
