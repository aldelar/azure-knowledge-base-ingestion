...
| Field | Written by |
|---|---|
| `id`, `conversationId` | First writer (agent or web app) |
...
**Users** are stored in the same container as separate documents with a special partition:
...
│                                                                         │
│  Partition: conversationId = "abc-123"                                  │
│  ┌───────────────────────────────────────────────────────┐              │
│  │  id: "abc-123"                                        │              │
...
│  └───────────────────────────────────────────────────────┘              │
│                                                                         │
│  Partition: conversationId = "__users__"                                │
│  ┌───────────────────────────────────────────┐                          │
...
`_read_thread_doc(thread_id)` performs a direct point read using `partition_key=thread_id` (since `conversationId == thread_id`):
...
| **Partition key** | `/conversationId` (thread UUID; `__users__` for user docs) |
