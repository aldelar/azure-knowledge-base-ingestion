...
> This diagram illustrates a secure network architecture for connecting on-premises resources to Azure Cognitive Search using Azure services. The flow starts from an on-premises environment, connects via ExpressRoute (ER) Private Peering to an ER Gateway, then routes through an App Service within a Virtual Network (10.0.0.0/16). Internet access is denied at this stage, ensuring traffic is private. The App Service communicates with Azure Cognitive Search, which is isolated within its own virtual network, protected by a firewall.
> **UI Elements**: On-premises icon, ER Private Peering icon, ER Gateway icon, App Service icon, Virtual Network label (10.0.0.0/16), Deny Internet globe icon, Firewall icon, Azure Cognitive Search icon, Azure Cognitive Search Virtual Network label
> **Navigation Path**: On-premises > ER Private Peering > ER Gateway > App Service (Virtual Network) > Firewall > Azure Cognitive Search (Virtual Network)
...
For more stringent security, you can establish a private endpoint for Azure AI Search allows a client on a [virtual network](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview) to securely access data in a search index over a Private Link.
...
> This diagram illustrates a secure network architecture for connecting an on-premises environment to Azure Cognitive Search using Azure Private Link. The flow starts from an on-premises network, connects via ExpressRoute (ER) Private Peering to an ER Gateway, then to a Virtual Machine (VM) within a Virtual Network. The VM accesses Azure Cognitive Search through a Private Endpoint (with IP 10.0.0.5), which is secured by Private Link. Internet access is denied at both the on-premises and Azure Cognitive Search sides, ensuring all traffic remains private. The diagram highlights the separation between the on-premises network, the Azure Virtual Network, and the Azure Cognitive Search Virtual Network.
> **UI Elements**: On-premises icon, ER Private Peering, ER Gateway, VM icon, Private Endpoint (10.0.0.5), Private Link, Azure Cognitive Search cloud icon, Deny Internet icons, Virtual Network boundary, Azure Cognitive Search Virtual Network boundary
> **Navigation Path**: On-premises > ER Private Peering > ER Gateway > VM > Private Endpoint > Private Link > Azure Cognitive Search
