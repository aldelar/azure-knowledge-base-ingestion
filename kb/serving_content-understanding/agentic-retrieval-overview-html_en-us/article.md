...
> The image is a diagram explaining how to process a compound user query about a software patch (KB4048959). The central part of the image contains the query: 'what does KB4048959 fix and what systems is the patch compatible with?' This query is highlighted with different colors for emphasis. Surrounding the query are three callout boxes with arrows pointing to the query, each describing a best practice for handling such queries: using chat history for context, splitting queries for different information requirements, and correcting spelling in sentence context.
> **UI Elements**: Central highlighted query text, Callout box: 'Use chat history for context (the user said earlier it was a security fix)', Callout box: 'Split queries for different information requirements, resolving co-references', Callout box: 'Correcting spelling is much more effective if done in sentence context', Arrows connecting callout boxes to the query
> **Navigation Path**: N/A (diagram, not a software UI navigation)
...
> This image is a flowchart illustrating 'Agentic methods applied to retrieval.' It shows the process of handling a user query and conversation turns through query planning, fan-out query execution, and results merging. The flow starts with a user query and conversation turns, which are fed into a 'Query planning' module. This module generates multiple search queries (Search query 1 to Search query n), each of which is executed and produces results at two levels (L1 and L2). These results are then merged into a final output. At the bottom, it highlights that this process can be managed in a single LLM call, with additional capabilities such as using conversation history, correcting spelling, decomposing queries, and paraphrasing queries.
> **UI Elements**: User query input, Conversation turns input, Query planning module, Search query outputs (1 to n), L1 and L2 result levels, Merged results output, Single LLM call annotation, Process steps: Query planning, Fan-out query execution, Results merging
> **Navigation Path**: User query/Conversation turns > Query planning > Search queries (1 to n) > L1/L2 results > Merged results
