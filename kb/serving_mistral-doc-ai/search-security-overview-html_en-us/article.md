...
> **Description:**  
> This image is a network architecture diagram illustrating how on-premises infrastructure can securely connect to Azure Cognitive Search using ExpressRoute Private Peering and Azure Virtual Networks. The flow shows traffic moving from an on-premises environment, through an ExpressRoute Gateway and an App Service hosted inside an Azure Virtual Network with outbound internet access denied, then secured via a firewall, before finally reaching Azure Cognitive Search in its own [virtual network](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview). This setup is designed to enhance security and isolate traffic from the public internet.
> 
> **UIElements:**  
> None
> 
...
> Description:  
> This image illustrates a network architecture for securely connecting an on-premises environment to Azure Cognitive Search using Azure Private Link. The diagram shows a setup where traffic flows from an on-premises network through ExpressRoute private peering and an ER Gateway, into a virtual network containing a VM and a private endpoint, which connects to Azure Cognitive Search via Private Link. Internet traffic is explicitly denied at various points, ensuring that access to Azure Cognitive Search is restricted to private connections only.
> 
> UIElements:  
> None.
> 
