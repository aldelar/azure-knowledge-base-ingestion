...
> This image is a network architecture diagram showing a secure connection from an on-premises environment to Azure Cognitive Search using Azure services. The flow starts from an on-premises network, connects via ExpressRoute (ER) Private Peering to an ER Gateway, then to an App Service within a Virtual Network (10.0.0.0/16). Internet access is denied at this stage. The App Service communicates through a firewall to Azure Cognitive Search, which is hosted in its own virtual network. The diagram emphasizes secure, private connectivity and restricted internet access.
