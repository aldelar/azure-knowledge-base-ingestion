...
> This image is an architecture diagram illustrating a secure, hybrid connectivity setup between an on-premises environment and Azure Cognitive Search. The flow starts from an on-premises location connected via ExpressRoute Private Peering (ER Private Peering) through an ER Gateway, which then communicates with an application service hosted in a [virtual network](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview) (10.0.0.0/16). Internet access is denied at this stage to ensure security. The app service then interfaces with Azure Cognitive Search, which is protected behind a firewall and hosted within its own virtual network. The diagram highlights secure, private communication from on-premises to Azure Cognitive Search with no direct internet access allowed
...
> The image depicts a secure network architecture for accessing Azure Cognitive Search using Azure Private Link. Data flows from an on-premises environment through ER (ExpressRoute) Private Peering and an ER Gateway into a virtual machine (VM) within an Azure Virtual Network (VNet). The VM communicates with Azure Cognitive Search through a Private Endpoint (with an internal IP address 10.0.0.5), traversing the Private Link, which isolates traffic from the public internet. Internet access is explicitly denied both before and after Azure Cognitive Search, ensuring all data transfer remains private and internal between the connected VNets
