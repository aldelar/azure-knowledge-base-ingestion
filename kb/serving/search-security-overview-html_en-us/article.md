...
> The image is a network architecture diagram illustrating a secure connection from an on-premises environment to Azure Cognitive Search via ExpressRoute private peering and a gateway. Traffic flows from the on-premises site through an ExpressRoute Private Peering link into an ExpressRoute Gateway within an Azure [virtual network](https://learn.microsoft.com/en-us/azure/virtual-network/virtual-networks-overview), then to an Azure App Service, and finally to the Azure Cognitive Search service, which resides within its own virtual network. Internet access is explicitly denied in this routing setup, ensuring all data transmission is kept private and secure
...
> This image is a network topology diagram illustrating how to securely connect an on-premises environment to Azure Cognitive Search using Azure Private Link. The diagram shows traffic flow from an on-premises network through ExpressRoute Private Peering and an ExpressRoute Gateway to a virtual machine in a virtual network, which then accesses Azure Cognitive Search via a Private Endpoint and Private Link. Internet access is denied at multiple points to ensure a secure, private connection
