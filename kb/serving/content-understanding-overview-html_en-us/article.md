...
> This image visually explains the "Content Understanding Framework," a system that processes various input types (documents, images, video, audio) through multiple analyzing stages—content extraction, segmentation, field extraction, and postprocessing—to generate structured output (in Markdown or JSON schema). The framework incorporates both specialized AI models (for OCR, layout, transcription) and foundational models (such as GPT-4o, 4.1 family, and embeddings), and its structured outputs are then made available to end-use applications like search, agents, databases, copilots, apps, and Fabric
> **UI Elements**: - Labeled rounded buttons for input sources: Documents, Image, Video, Audio
> - Labeled sections (boxes) for stages: Content Extraction, Segmentation, Field Extraction, Postprocessing, Structured Output
> - Labeled area for Specialized AI Models and Foundational Models, with a “New” badge
> - Vertical icon menu for output destinations: Search, Agents, Databases, Copilots, Apps, Fabric
> **Navigation Path**: N/A (The image is a conceptual diagram, not a screenshot of a software interface with a navigational path.)
