...
> The image is a diagram explaining how to process a user query about a software patch (KB4048959). The central box contains the query: 'what does KB4048959 fix and what systems is the patch compatible with?' Surrounding the query are three callout boxes with arrows pointing to the query, each providing a tip for handling the query: using chat history for context, splitting queries for different information requirements, and correcting spelling in sentence context. The query itself is highlighted with different colors for emphasis on key phrases.
...
> This image is a flowchart diagram illustrating how agentic methods are applied to information retrieval. It is divided into three main stages: Query planning, Fan-out query execution, and Results merging. The process starts with a user query and conversation turns, which feed into the Query planning stage. Query planning generates multiple search queries (Search query 1 to Search query n). Each search query is executed, producing results at different levels (L1, L2). These results are then merged in the final stage. At the bottom, a note highlights that a single LLM call can use conversation history, correct spelling, decompose queries, and paraphrase queries as needed.
