...
> This image is a conceptual illustration showing best practices for processing a compound user query about a software patch (KB4048959). The central query is highlighted: 'what does KB4048959 fix and what systems is the patch compatible with?' Surrounding the query are three annotated suggestions: using chat history for context, splitting queries for different information requirements, and correcting spelling in sentence context. The image visually demonstrates how to break down and enhance the understanding of complex user questions in a technical support or AI assistant scenario.
...
> This image is a conceptual diagram illustrating 'Agentic methods applied to retrieval' in the context of large language models (LLMs). It shows a process flow divided into three main stages: Query planning, Fan-out query execution, and Results merging. The flow starts with a user query and conversation turns, which feed into a 'Query planning' module. This module generates multiple search queries (Search query 1 to Search query n), each of which is executed in parallel. The results from each search query are processed at two levels (L1 and L2), and then merged into a final set of results. At the bottom, a note highlights that a single LLM call can use conversation history for context, correct spellings, decompose queries, and paraphrase queries as needed.
