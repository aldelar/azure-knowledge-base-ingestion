...
> This image is a network architecture diagram illustrating a secure connection from an on-premises environment to Azure Cognitive Search using Azure Private Link. The flow starts from an on-premises network, connects via ExpressRoute (ER) Private Peering to an ER Gateway, then to a Virtual Machine (VM) within a Virtual Network (10.0.0.0/16). The VM accesses a Private Endpoint (10.0.0.5), which is connected to Azure Cognitive Search through a Private Link. Internet access is denied at both the on-premises and Azure Cognitive Search sides, ensuring all traffic remains private. The diagram highlights the use of private networking to securely access Azure Cognitive Search without exposing resources to the public internet.
